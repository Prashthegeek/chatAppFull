...
import { FormControl } from "@chakra-ui/form-control";
import { Input } from "@chakra-ui/input";
import { Box, Text, Image, IconButton, Spinner, useToast } from "@chakra-ui/react";
import { ArrowBackIcon, ArrowRightIcon, AttachmentIcon, CloseIcon } from "@chakra-ui/icons";
import { useEffect, useState } from "react";
import axios from "axios";
import { getSender, getSenderFull } from "../config/ChatLogics";
...
const ENDPOINT = "https://chatappfull.onrender.com";
  ...
  useEffect(() => {
    socket.on("message received", (newMessageRecieved) => {
      ...
      if (!selectedChatCompare || selectedChatCompare._id !== newMessageRecieved.chat._id) {
        if (!notification.includes(messageWithTimestamp)) {
          setNotification((prev) => [messageWithTimestamp, ...prev]);
          setFetchAgain(!fetchAgain);
        }
      } else {
  ...
  return (
    <>
      {selectedChat ? (
        <>
          ...
          <Box d="flex" flexDir="column" justifyContent="flex-end" p={3} bg="#E8E8E8" w="100%" h="100%" borderRadius="lg" overflowY="hidden">
            ...
            <FormControl id="first-name" isRequired mt={3}>
              ...
              {showEmojiPicker && (
                <Box position="absolute" bottom="60px" right="20px">
                  <Picker data={data} onEmojiSelect={addEmoji} theme="light" style={{ width: '200px', height: '200px' }} />
                </Box>
              )}
              {file && (
                <Box mb={2} bg="gray.200" p={2} borderRadius="md" position="relative" maxW="200px" maxH="200px">
                  <Text>{file.name}</Text>
                  <IconButton
                    position="absolute"
                    top={1}
        ...
        <Text fontSize="3xl" color="gray.600" textAlign="center" h="100%">
